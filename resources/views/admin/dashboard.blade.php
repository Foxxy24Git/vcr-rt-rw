        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                <article class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-3 text-3xl font-bold tracking-tight text-gray-900">{{ number_format((int) ($activeUsersCount ?? 0)) }}</p>
                </article>
                ...
                @foreach ($kpis as $kpi)
                    <article class="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <p class="mt-3 text-2xl font-semibold tracking-tight text-gray-900">{{ $kpi['value'] }}</p>
                    </article>
                @endforeach
            </div>

            @if (($overloadedResellers ?? collect())->isNotEmpty())
                <section class="rounded-2xl border border-red-300 bg-red-50 p-6">
                    <h3 class="text-base font-semibold text-red-800">&#9888; Reseller Overload Alert</h3>

                    <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                        @foreach ($overloadedResellers as $item)
                            <article class="rounded-xl border border-red-200 bg-white/80 p-4">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <h3 class="text-base font-semibold text-gray-900">Active Users per Reseller</h3>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <h3 class="text-base font-semibold text-gray-900">Network Utilization</h3>
                ...
                <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                    <article class="rounded-xl border border-gray-100 bg-gray-50 p-4">
                        ...
                        <p class="mt-1 text-2xl font-bold text-gray-900">{{ number_format((float) ($estimatedMbps ?? 0), 2, ',', '.') }} Mbps</p>
                    </article>
                    <article class="rounded-xl border border-gray-100 bg-gray-50 p-4">
                        <p class="text-xs uppercase tracking-wide text-gray-500">Total Capacity</p>
                        <p class="mt-1 text-2xl font-bold text-gray-900">{{ number_format((float) ($networkCapacityMbps ?? 0), 2, ',', '.') }} Mbps</p>
                    </article>
                    <article class="rounded-xl border border-gray-100 bg-gray-50 p-4">
                        <p class="text-xs uppercase tracking-wide text-gray-500">Utilization</p>
                        <p class="mt-1 text-2xl font-bold text-gray-900">{{ number_format($utilization, 1, ',', '.') }}%</p>
                    </article>
                </div>
            ...
            <section class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <h3 class="text-base font-semibold text-gray-900">Active Users Trend (Last 24h)</h3>
